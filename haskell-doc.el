...
(defconst haskell-doc-version "$Revision: 1.18 $"
...
(defun haskell-doc-extract-types (url)
  (with-temp-buffer
    ...
    (goto-char (point-min))
    (re-search-forward "^module +\\([[:alnum:]]+\\)")
    (let ((module (match-string 1)))
      ...
      (let* ((elems nil)
             (re (concat
                 ...
                 ;; weed out entries like "ratPrec = 7 :: Int".
                 "\\([^ \t\n]+\\(?:[ \t]*,[ \t]*[^ \t\n]+\\)*\\)"
                 ;; And finally a type.
                 "[ \t]*::[ \t]*\\(.*\\)"))
             (re-class (concat "^[^ \t]\\|" re))
          ...
          (cond
           ...
           ((match-end 4)
            (let ((type (match-string 4))
                  (vars (match-string 3)))
              (if (match-end 2)
                  (if curclass (setq type (concat curclass " => " type))
                    ;; It's actually not an error: just a type annotation on
                    ;; some local variable.
                    ;; (error "Indentation outside a class in %s: %s" module vars)
                    nil)
                (setq curclass nil))
              (dolist (var (split-string vars "[ \t]*,[ \t]*" t))
...
(defun haskell-doc-fetch-lib-urls (base-url)
  (with-temp-buffer
    ...
    (goto-char (point-min))
    (let ((libs '("standard-prelude.html")))
